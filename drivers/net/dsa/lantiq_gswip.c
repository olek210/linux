			   ...
			   struct ethtool_regs *regs, void *_p)
{
	...
	if (!dsa_is_cpu_port(priv->ds, port)) {
		...
		p[6] = gswip_mdio_r(priv, GSWIP_MDIO_EEEp(port));
		p[7] = gswip_mii_r(priv, GSWIP_MII_CFGp(port));
	}

	switch (port) {
	case 0:
		p[8] = gswip_mii_r(priv, GSWIP_MII_PCDU0);
		break;
	case 1:
		p[8] = gswip_mii_r(priv, GSWIP_MII_PCDU1);
		break;
	case 5:
		p[8] = gswip_mii_r(priv, GSWIP_MII_PCDU5);
		break;
	default:
		break;
	}

	p[9] = gswip_switch_r(priv, GSWIP_PCE_PCTRL_0p(port));
	p[10] = gswip_switch_r(priv, GSWIP_PCE_VCTRL(port));
	p[11] = gswip_switch_r(priv, GSWIP_PCE_DEFPVID(port));
	p[12] = gswip_switch_r(priv, GSWIP_MAC_FLEN);
	p[13] = gswip_switch_r(priv, GSWIP_MAC_PSTATp(port));
	p[14] = gswip_switch_r(priv, GSWIP_MAC_CTRL_0p(port));
	p[15] = gswip_switch_r(priv, GSWIP_MAC_CTRL_2p(port));
	p[16] = gswip_switch_r(priv, GSWIP_FDMA_PCTRLp(port));
	p[17] = gswip_switch_r(priv, GSWIP_SDMA_PCTRLp(port));
}
